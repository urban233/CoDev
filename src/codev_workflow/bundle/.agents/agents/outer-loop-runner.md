...
specialists in parallel, each with the exact PR diff, work item, and
...
record --id <work-item-id> --round <round> --role reviewer
...
- `ok_continue` — at least one finding was `address`-selected: use
  ...
  round — `codev work check` fills in every other dimension automatically
