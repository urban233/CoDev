...
7. Use `invoke_subagent` to start `code-audit-gate` against that exact head
   ...
   commit --id <work-item-id> --message <summary>` — a plain commit, not
...
8. Use `invoke_subagent` to start `lightweight-reviewer` in a fresh task
   ...
   cleanup — and work item, plus `code-audit-gate`'s summary from step 7 if
   it changed anything. This pass is deliberately narrow — correctness and
   intent-match against the work item, plus independent re-verification
