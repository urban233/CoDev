...
2. Coarse fallback: a repo-wide, branch-name-substring match against
   `docs/features/*/design.md` or `docs/codev/features/*/design.md`, for
   planning work that predates a `codev task start` (e.g. still drafting a
   brief/design) or a branch that never went through `codev git branch`.
"""
...
def _has_matching_spec(repo_root: Path, slug: str) -> bool:
    for pattern in _SPEC_GLOBS:
        for match in repo_root.glob(pattern):
            directory_name = match.parent.name.lower()
            if slug and (slug in directory_name or directory_name in slug):
                return True
    return False
