...
`.claude/settings.json` and three guardrail hooks — a category no other adapter has:
...
enforcing rolling-wave planning's detail-only-the-current-wave rule; `require_small_change.py`
asks when a task's diff exceeds its `review.max_lines`/`review.max_files` budget at
`codev git open-pr`. All three fail open on any internal error and log their decisions to
a local, gitignored gate-decision log.
